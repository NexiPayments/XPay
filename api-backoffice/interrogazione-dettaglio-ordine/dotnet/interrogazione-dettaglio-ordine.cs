...
namespace XPay
{
    class SampleXPayRequest
    {
        static void Main(string[] args)
        {
            ...
            string codTrans = "<CODICE TRANSAZIONE>"; // Codice della transazione di cui si cercano i dettagli
            string timeStamp = (Math.Round((DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalMilliseconds)).ToString();            

            // Calcolo MAC
            string mac = HashMac("apiKey=" + apiKey + "codiceTransazione=" + codTrans + "timeStamp=" + timeStamp + chiaveSegreta);
