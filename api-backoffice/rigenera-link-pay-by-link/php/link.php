...
if ($connection) {
    ...
    $requestURI = "ecomm/api/bo/rigeneraPayMail"; // URI
    
    // Parametri per calcolo MAC
    $apiKey = "<ALIAS>"; // Alias fornito da Nexi
    $chiaveSegreta = "<CHIAVE SEGRETA PER CALCOLO MAC>"; // Chiave segreta fornita da Nexi
    $codiceTransazione = ""; // Codice della transazione
    $nuovoCodiceTransazione = "";// Nuovo codice transazione a cui associare il link di pagamento.	
    $timeout = 10000; // Durata in ore del link di pagamento che verrà generato 
    $timeStamp = (time()) * 1000;
    ...
    if (json_last_error() === JSON_ERROR_NONE) {
        ...
        if ($risposta['mac'] == $MACrisposta) {
            ...
            if ($risposta['esito'] == 'OK') {
                ...
                echo "<a href ='" . $risposta['payMailUrl'] . "'>VAI AL LINK</a><br>";
                echo "Codice transazione: " . $risposta['codiceTransazione'];
            } else {
