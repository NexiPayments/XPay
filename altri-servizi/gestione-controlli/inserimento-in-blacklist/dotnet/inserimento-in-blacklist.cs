...
namespace XPay
{
    class SampleXPayRequest
    {
        static void Main(string[] args)
        {
            ...
            // URL + URI
            string requestUrl = "https://int-ecommerce.cartasi.it/" + "ecomm/api/blacklist/aggiungi";
            ...
            string valore = "<NUMERO>"; // NumeroContratto oppure codice fiscale
            string descrizione = ""; // Motivo inserimento in blacklist
            string timeStamp = (Math.Round((DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalMilliseconds)).ToString();
            
            // Calcolo MAC
            string mac = HashMac("apiKey=" + apiKey + "tipo=" + tipo + "valore=" + valore + "descrizione=" + descrizione + "timeStamp=" + timeStamp + chiaveSegreta);
            ...
            requestParams["descrizione"] = descrizione;
