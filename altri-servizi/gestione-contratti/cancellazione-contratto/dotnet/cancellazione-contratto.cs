...
namespace XPay
{
    class SampleXPayRequest
    {
        static void Main(string[] args)
        {
            ...
            // URL + URI
            string requestUrl = "https://int-ecommerce.cartasi.it/" + "ecomm/api/contratti/cancellaContratto";
            ...
            string numContratto = "<NUMERO CONTRATTO>"; // Numero del contratto da cancellare
            string timeStamp = (Math.Round((DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalMilliseconds)).ToString();

            // Calcolo MAC
            string mac = HashMac("apiKey=" + apiKey + "numeroContratto=" + numContratto + "timeStamp=" + timeStamp + chiaveSegreta);
